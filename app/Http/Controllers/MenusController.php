...
class MenusController extends Controller
{
    ...
    public function __construct(Application $app, Audit $audit)
    {
        ...
        if (\Auth::check()) {
            $this->language = \Auth::user()->settings()->get('locale');
        }
    }
    ...
    public function index()
    {
        ...
        $parents = Menu::where('separator', '0')->orderBy('label', 'ASC')->orderBy('id', 'ASC')->get()->pluck('label', 'id');
        // Convert to array.
     ...
     * @return string
     */
    public function getLabelByLanguage($menu)
    {
        $variable = $this->language;

        switch ($variable) {
            case 'np':
                $lang = $menu->nepali_label;
                break;
            case 'es':
                $lang = $menu->spanish_label;
                break;
            default:
                $lang = $menu->label;
                break;
    ...
    private function menusOrmToJsTreeJson(Collection $menusCol)
    {
        $jsTreeCol = $menusCol->map(function ($item, $key) {
            ...
            $label = $this->getLabelByLanguage($item);
            $icon = $item->icon;
    ...
    public function save(Request $request)
    {
        ...
        if ($id) { // If $id is not null or blank we must be editing.
            ...
            if (! $menuItem->isEditable()) {
                ...
                // validate attributes.
                $this->validate($request, ['name' => 'required|unique:menus,name,'.$id,
                                                    'label' => 'required',
                ]);
            ...
            unset($attributes['id']);
            // Validate attributes.
            $this->validate($request, ['name' => 'required|unique:menus',
                                                'label' => 'required',
            ]);
    ...
    public function getModalDelete($id)
    {
        ...
        if (! $menu->isdeletable()) {
            ...
            $modal_route = route('admin.menus.delete', ['id' => $menu->id]);
        }
