...
//        //TODO: Warn of any routes in our DB that is not used in the app.
        ...
        $routes = $this->route->with('permission')
                              ->orderBy('path', 'ASC')
                              ->orderBy('method', 'ASC')
                              ->paginate(20);
...
//        array_unshift($perms, '');
    ...
    {
        $this->validate($request, ['name'          => 'required|unique:routes',
    ...
    {
        $this->validate($request, ['name'          => 'required|unique:routes,name,'.$id,
                                            'action_name'   => 'required',
                                            'method'        => 'required',
                                            'path'          => 'required',
        ]);
    ...
    public function getModalDelete($id)
    {
        ...
        $modal_route = route('admin.routes.delete', ['id' => $route->id]);
    ...
    public function searchByName(Request $request)
    {
        ...
        $routes = $this->route->where('name', 'LIKE', '%'.$query.'%')
                     ->orWhere('path', 'LIKE', '%'.$query.'%')
                     ->orWhere('action_name', 'LIKE', '%'.$query.'%')
                     ->get();
