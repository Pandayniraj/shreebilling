...
//    public static function convertToLocalDateTime($utcDate)
    ...
    {
        $time_zone = self::getUserOrAppOrDefaultSetting('time_zone', 'app.time_zone', 'UTC');
        $time_format = self::getUserOrAppOrDefaultSetting('time_format', 'app.time_format', '24');
    ...
    public static function getUserOrAppOrDefaultSetting($userKey, $appKey = null, $default = null)
    {
        ...
        if (null == $setting) {
            // $setting = \App\Models\Setting::get($appKey, $default);
        }
    ...
    public static function formatPath($path, $line)
    {
        ...
        if ($linkFormat = $fileLinkFormat) {
            $link = strtr(self::escapeHtml($linkFormat), ['%f' => $path, '%l' => (int) $line]);
