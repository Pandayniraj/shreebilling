...
trait MenuHandlerTrait
{
    ...
    public function getMenuItem($input)
    {
        ...
        try {
            ...
            $menuItem = $this->menuRepository->where('name', $input)->first();
            if ($menuItem instanceof Menu) {
                return $menuItem;
            }
        } catch (QueryException $ex) {
...
//
        ...
        $leaf = $this->menuRepository->where('url', $currentUrl)->first();
        if ($leaf instanceof Menu) {
            return $leaf;
        }
    ...
    public function getVariables(Menu $menu, $variables = [])
    {
        ...
        $label = $this->getLabelByLanguage($menu);
