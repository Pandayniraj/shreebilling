    ...
    <div class="col-sm-12">
        <div class="panel panel-custom" data-collapsed="0">
            ...
            <div class="panel-body">
                <form id="attendance-form" role="form" enctype="multipart/form-data" action="/admin/biometricattendence/" method="get" class="form-horizontal form-groups-bordered">
                    {{ csrf_field() }}
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <select required name="emp_id" id="user_id" class="form-control select_box">
                                <option value="">Select Employee</option>
                                @foreach($BioUsers as $uv)
                                @if($uv->machine_userid == $id)
                                <option value="{{ $uv->machine_userid }}" selected>{{$uv->name}}({{$uv->    machine_userid}})</option>
                                @else
                                 <option value="{{ $uv->machine_userid }}" >{{$uv->name}} ({{$uv->machine_userid}})</option>
                                @endif
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="panel-heading">
                    <h3 class="panel-title">
                        ...
                        <div class="pull-right hidden-print">
                            <a href="/admin/time_history/generatePDF/{{ $user_id }}/{{ $date_in }}" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="top" title="" data-original-title="PDF" target="_blank"><span><i class="fa fa-file-pdf"></i></span></a>
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td  style="background: rgba(233, 237, 228, 0.73);font-weight: bold">  
                                {{ date('dS M y', strtotime($hv[0]['datetime'])) }}
                            </td>
                            <td ></td>
                            <td ></td>
                            <td ></td>
                        ...
                        <tr>
                            @for($i=0; $i<4;$i++)
                            <td>@if($hv[$i]['datetime'])
                            {{ date('h:i a', strtotime($hv[$i]['datetime'])) }}
                            ({{$hv[$i]['verification_mode']}})
                            @else
                            @endif</td>
                            @endfor
                            <td>{{ $hv[0]['ip_address'] }}</td>

                            <?php
                            $totalDuration = 0;
                            if($hv[1]){
                $startTime = \Carbon\Carbon::parse(date('Y-m-d H:i:s', strtotime($hv->date_in.' '.$hv[0]['datetime'])));
                $finishTime = \Carbon\Carbon::parse(date('Y-m-d H:i:s', strtotime($hv->date_out.' '.$hv[1]['datetime'])));
                                //dd($startTime.' '.$finishTime);
                                $totalDuration = $finishTime->diffInSeconds($startTime);
                            }
                    if($hv[3]){
                           $startTime = \Carbon\Carbon::parse(date('Y-m-d H:i:s', strtotime($hv->date_in.' '.$hv[2]['datetime'])));
                $finishTime = \Carbon\Carbon::parse(date('Y-m-d H:i:s', strtotime($hv->date_out.' '.$hv[3]['datetime'])));
                                //dd($startTime.' '.$finishTime);
                            ...
                            ?>
                            <td>{{ $hour.':'.$minute }}</td>
                            <td class="hidden-print"><a href="/admin/time_history/edit_time/{{ $hv->clock_id }}" class="btn btn-primary btn-xs" title="Edit" data-toggle="modal" data-placement="top" data-target="#myModal"><span class="fa fa-pen-square"></span></a></td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
