...
<section class="content-header" style="margin-top: -35px; margin-bottom: 20px">
            ...
            <p> Entry for Advance Salary payment</p>
    ...
    <div class="col-lg-12">
        <div class="row">
            ...
            <div class="col-sm-9">
                ...
                <a class="btn btn-primary btn-sm pull-right" href="" data-toggle="modal" data-placement="top" data-target="#eventModal">
                          <i class="fa fa-plus"> </i> New Advance Salary
                      </a>
                                ...
                                <table class="table table-bordered table-hover">
                                    <thead>
                                        <tr class="bg-success">
                                            ...
                                            <th class="col-sm-2">Deduct Month</th>
                                            <th class="col-sm-2">Request Date</th>
                                    ...
                                    <tbody>
                                        @foreach($advanceSalaries as $ask => $asv)
                                         <?php
                                                $m_temp = explode('-', $asv->deduct_month);
                                            ?>
                                            @if($m_temp[1] == $j)
                                            <?php $count_flag++ ?>   
                                            <tr>
                                                ...
                                                <td>{{ $asv->advance_amount }}</td>
                                                <td>{{ $asv->deduct_month }}</td>
                                                <td>{{ $asv->request_date }}</td>
                                                <td>{{ $asv->approveBy->first_name }}</td>
                                                <td>
                                                    <span class="label @if($asv->status == 0) label-warning @elseif($asv->status == 1) label-success @elseif($asv->status == 2) @else label-danger @endif">@if($asv->status == 0) Pending @elseif($asv->status == 1) Accepted @elseif($asv->status == 2) Rejected @else Paid @endif</span>
                                                </td>
                                                <td>
                                                    <button class="btn btn-primary btn-xs event_edit" title="Edit" data-placement="top" data-id="{{ $asv->advance_salary_id }}"><span class="fa fa-edit"></span></button>
                                                    <a href="/admin/payroll/advance_salary_delete/{{ $asv->advance_salary_id }}" class="btn btn-danger btn-xs" title="" data-toggle="tooltip" data-placement="top" onclick="return confirm('You are about to delete a record. This cannot be undone. Are you sure?');" data-original-title="Delete"><i class="fa fa-trash"></i></a>
                                                </td>
                                            </tr>
                                             @endif
                                        @endforeach
