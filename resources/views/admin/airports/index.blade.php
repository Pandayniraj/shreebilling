    ...
    <div class='col-md-12'>
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <a class="btn btn-default btn-sm" href="{!! route('admin.airport.create') !!}" title="{{ trans('admin/fiscalyear/general.button.create') }}">
                    ...
                    <table class="table table-hover table-bordered" id="FiscalYear-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{!! $v->code !!}</td>
                                <td><a href="{{route('admin.airport.show',$v->id)}}" size="16px;">{!! $v->name !!}</a></td>
                                <td>{{$v->cityCode}}</td>
                                <td>{{ $v->cityName}}</td>
                                <td>{{ $v->countryCode}}</td>
                                <td>{{ $v->countryName}}</td>
                                <td>
                                    ...
                                    <a href="{!! route('admin.airport.edit', $v->id) !!}" title="{{ trans('general.button.edit') }}"><i class="fa fa-edit"></i></a>
                                    @else
                                    <i class="fa fa-edit text-muted" title="{{ trans('admin/fiscalyear/general.error.cant-edit-this-FiscalYear') }}"></i>
                                    @endif
                                    @if( $v->isDeletable() )
                                    <a href="{!! route('admin.airport.confirm-delete', $v->id) !!}" data-toggle="modal" data-target="#modal_dialog" title="{{ trans('general.button.delete') }}"><i class="fa fa-trash deletable"></i></a>
                                    @else
