...
<form method="post" action="{{route('admin.appraisal.edit',$appraisal->id)}}">
    {{ csrf_field() }}
                            ...
                            <div class="col-sm-5">
                                <select required name="evaluator_id" id="evaluator_id" class="form-control select_box">
                                    ...
                                    <optgroup label="{{$dep->deptname}}">
                                        ...
                                        ?>
                                        @foreach($emp as $e)
                                        <option value="{{$e->id}}" @if($appraisal->evaluator_id==$e->id) selected @endif>
                                            {{ucfirst(trans($e->username))}}({{$e->designations}})
                                        </option>
                                        @endforeach
                                    </optgroup>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <div class="input-group">
                                    <input required="" type="text" class="form-control date_in" value="{{ $appraisal->appraisal_month }}" name="appraisal_month" id="appraisal_month">
                                ...
                                <div class="row">
                                    <div class="form-group" id="border-none">
                                        ...
                                        <div class="col-sm-4">
                                            ...
                                            @foreach($appraisal->appraisalData as $adk => $adv)
                                                @php array_push($arrayData, $adv->appraisal_data_field.':'.$adv->appraisal_data_field_key); @endphp
                                            @endforeach
                                            <select name="appraisal_marks[{{$aotv->id}}][{{$obv->id}}]" class="form-control" required>
                                            @foreach($qw as $key=>$tm) 
                                                @php $val = $aotv->id.":".$key; @endphp
                                                <option value="{{round(($key*$obv->marks*0.2), 2).':'.$key}}" @if(in_array($val, $arrayData)) selected @endif>{{$tm}}</option>
                                            @endforeach
                                            </select>
                                        </div>
                                    </div>
                                            ...
                                            <div class="form-group" id="border-none">
                                                ...
                                                <div class="col-sm-4">
                                                    ...
                                                    @foreach($appraisal->appraisalData as $adk => $adv)
                                                        @php array_push($arrayData, $adv->appraisal_data_field.':'.$adv->appraisal_data_field_key); @endphp
                                                    @endforeach
                                                    <select name="appraisal_marks[{{$aottv->objectiveType->id}}][{{$obv->id}}]" class="form-control" required>
                                                    @foreach($qw as $key=>$tm) 
                                                        @php $val = $aotv->id.":".$key; @endphp
                                                        <option value="{{round(($key*$obv->marks*0.2), 2).':'.$key}}" @if(in_array($val, $arrayData)) selected @endif>{{$tm}}</option>
                                                    @endforeach
                ...
                <div class="row">
                    <div class="panel panel-custom">
                        <div class="col-md-4">
                            ...
                            <textarea class="form-control" name="line_manager_comment" id="line_manager_comment" placeholder="Write Comment">{{ $appraisal->line_manager_comment }}</textarea>
                        </div>
                        <div class="col-md-4">
                            <label for="line_manager_score" class="control-label">Line Manager Score</label>
                            <input type="number" class="form-control date_in" value="{{ $appraisal->line_manager_score }}" name="line_manager_score" id="line_manager_score">
                        </div>
                        <div class="col-md-4">
                            <label for="line_manager_rating" class="control-label">Line Manager Rating</label>
                            <input type="number" class="form-control date_in" value="{{ $appraisal->line_manager_rating }}" name="line_manager_rating" id="line_manager_rating">
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
