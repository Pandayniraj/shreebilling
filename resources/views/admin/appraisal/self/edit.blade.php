...
<form method="post" action="{{route('admin.self.appraisal.edit',$appraisal->id)}}">
    {{ csrf_field() }}
        ...
        <div class="panel-heading">
                ...
                <div class="col-sm-12">
                    <div class="form-horizontal form-groups-bordered">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <select name="template_id" id="template_id" class="form-control select_box" @if($appraisal->entry_by != \Auth::user()->id || (int)$appraisal->marks_by_evaluator != 0) disabled @endif>
            ...
            <div class="row">
                                            ...
                                            @if(isset($appraisal->selfAppraisalData))
                                                @foreach($appraisal->selfAppraisalData as $adk => $adv)
                                                    @php
                                                        ...
                                                        if($adv->appraisal_data_field == $aotv->id && $adv->appraisal_data_field_key == $obv->id && $adv->appraisal_by == \Auth::user()->id)
                                                        {
                ...
                <span id="template_data">
                    {{-- {{dd($appraisal->template->appraisalTempType->count())}} --}}
                    @if($appraisal->template->appraisalTempType->count()>0)
                        <div class="clearfix"></div>
                        @foreach($appraisal->template->appraisalTempType as $aott => $aottv)
                            @if(isset($aottv->objectiveType) && $aottv->objectiveType->objectives->count())
                            <div class="col-sm-6">
                                <div class="panel panel-custom">
                                    <div class="bg-info box-header">
                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group">
                        ...
                        @if($appraisal->form_level ==0 )
                        <input class="btn btn-info" id="btn-draft-save" type="submit" value="Save as Draft" name="Save">
                        @endif
                        <a class="btn btn-default" href="/admin/self-appraisals"> Cancel </a>
                    </div>
                </div>
            </div>
        </div>
