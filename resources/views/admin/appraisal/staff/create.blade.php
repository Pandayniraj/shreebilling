...
<form method="post" action="{{route('admin.staff.appraisal.store')}}">
    {{ csrf_field() }}
        ...
        <div class="panel-heading">
            <div class="row">
                <div class="col-sm-12">
                    <div class="form-horizontal form-groups-bordered">
                        ...
                        <div class="form-group">
                            <label class="col-sm-3 control-label">Send To<span class="required"> *</span></label>
                            <div class="col-sm-5">
                                <select name="send_to" style="width: 100%" class="form-control"   tabindex="-1" aria-hidden="true"
                                id='send_to'>
                                  <!--   <option value="">Select Request To</option> -->
                                    <option value="{{\Auth::user()->firstLineManger->id}}">{{\Auth::user()->firstLineManger->first_name}} &nbsp; {{\Auth::user()->firstLineManger->last_name}}</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <div class="input-group">
                                    <input required="" type="text" class="form-control date_in" value="{{ old('appraisal_from') }}" name="appraisal_from" id="appraisal_from">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-5">
                                <div class="input-group">
                                    <input required="" type="text" class="form-control date_in" value="{{ old('appraisal_to') }}" name="appraisal_to" id="appraisal_to">
                                    <div class="input-group-addon">
                                        <a href="#"><i class="fa fa-calendar"></i></a>
                                    </div>
                                </div>
            ...
            <div class="row">
                                ...
                                <div class="row" style="padding: 1rem 0.2rem;border: 1px solid #eee;margin-bottom: 1rem !important;margin: 0;">
                                    <div class="form-group" id="border-none">
                                        ...
                                        <div class="col-sm-12" style="margin-top: 1rem;">
                                            <textarea name="comments[{{$aotv->id}}][{{$obv->id}}]" class="form-control" placeholder="Enter your comment (limit:150 char)" maxlength="150" required></textarea>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                ...
                <div class="col-md-12">
                    <br>
                    <div class="form-group">
                        <input class="btn btn-primary" id="btn-submit-edit" type="submit" value="Send" name="Send">
                        <input class="btn btn-info" id="btn-draft-save" type="submit" value="Save as Draft" name="Save" >
...
<script type="text/javascript">
    ...
    $('form #employee_id').change(function(){
        let uid = $(this).val();
        if(!uid){
            $('#send_to').html('<option value="">Selet Line Manager</option>');
        }
        $.get(`/admin/get_line_manager/${uid}`,function(response){
            $('#send_to').html(`<option value="${response.first_line_mananger.id}">${response.first_line_mananger.first_name} ${response.first_line_mananger.last_name}</option>`);
        });
    });
</script>
@endsection