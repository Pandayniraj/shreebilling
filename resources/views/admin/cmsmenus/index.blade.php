    ...
    <div class='col-md-12'>
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <a class="btn btn-default btn-sm" href="{!! route('admin.cms.menus.create') !!}" title="{{ trans('admin/fiscalyear/general.button.create') }}">
                    ...
                    <table class="table table-hover table-bordered" id="FiscalYear-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{!! $v->name !!}</td>
                                <td>{{ $v->url}}</td>
                                <td>{{ ($v->parent_id) ? $v->parent->name : 'Root'}}</td>
                                <td>{{ $v->user->first_name}} {{ $v->user->last_name}}</td>
                                <td>
                                    @if( $v->isEditable() || $v->canChangePermissions() )
                                    <a href="{!! route('admin.cms.menus.edit', $v->id) !!}" title="{{ trans('general.button.edit') }}"><i class="fa fa-edit"></i></a>
                                    @else
                                    <i class="fa fa-edit text-muted" title="{{ trans('admin/fiscalyear/general.error.cant-edit-this-FiscalYear') }}"></i>
                                    @endif
                                    @if( $v->isDeletable() )
                                    <a href="{!! route('admin.cms.menus.confirm-delete', $v->id) !!}" data-toggle="modal" data-target="#modal_dialog" title="{{ trans('general.button.delete') }}"><i class="fa fa-trash deletable"></i></a>
                                    @else
