 ...
 function CategoryTree($parent_id=null,$sub_mark='',$ledgers_data){
    ...
    if(count($groups)>0){
    	foreach($groups as $group){
    		...
    		$ledgers= \App\Models\COALedgers::orderBy('code', 'asc')->where('group_id',$group->id)->where('reconciliation','!=',0)->where('org_id',\Auth::user()->org_id)->get(); 
          ...
          <div class="box">
            ...
            <div class="box-body">
              ...
              @if(!empty($ledgers_data))
				<a href="{{route('admin.reconcile.pdf',$ledgers_data->id)}}" class="btn btn-primary">PDF</a>
				<a href="/admin/reconcile/excel/{{$ledgers_data->id}}" class="btn btn-primary">Excel</a>
          	    <a href="{{route('admin.reconcile.print',$ledgers_data->id)}}" class="btn btn-primary">Print</a>
          	  @endif  
				 </div>
          </div>
      </div>
