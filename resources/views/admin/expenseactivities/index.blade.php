    ...
    <div class='col-md-12'>
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <a class="btn btn-default btn-sm" href="{!! route('admin.expense.activities.create') !!}" title="{{ trans('admin/fiscalyear/general.button.create') }}">
            ...
            <div class="box-body">
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-bordered" id="FiscalYear-table">
                        ...
                        <tbody>
                            @foreach($expenseActivity as $v)
                            <tr>
                                <td align="center">
                                    {{$v->id}}
                                </td>
                                <td>{!! $v->name !!}</td>
                                <td>{{ $v->ExpensePa->name}}</td>
                                <td>{{ $v->rr_budget }}</td>
                                <td>{{$v->poa_budget}}</td>
                                <td>
                                    @if( $v->isEditable() || $v->canChangePermissions() )
                                    <a href="{!! route('admin.expense.activities.edit', $v->id) !!}" title="{{ trans('general.button.edit') }}"><i class="fa fa-edit"></i></a>
                                    @else
                                    <i class="fa fa-edit text-muted" title="{{ trans('admin/fiscalyear/general.error.cant-edit-this-FiscalYear') }}"></i>
                                    @endif
                                    @if( $v->isDeletable() )
                                    <a href="{!! route('admin.expense.activities.confirm-delete', $v->id) !!}" data-toggle="modal" data-target="#modal_dialog" title="{{ trans('general.button.delete') }}"><i class="fa fa-trash deletable"></i></a>
                                    @else
