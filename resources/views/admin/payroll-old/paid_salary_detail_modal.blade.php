            ...
            <div class="row">
                            ...
                            <table class="table-hover">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>&nbsp;&nbsp;&nbsp;</td>
                                        <td>{!! date('F Y', strtotime($salary->payment_month)) !!}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="col-sm-12 form-horizontal panel-custom">
                    ...
                    <div class="panel-body">
                        <div class="">
                            ...
                            <p class="form-control-static">{{ $salary->salary_grade }}</p>
                        </div>
                        <div class="">
                            <label for="field-1" class="col-sm-5 control-label"><strong>Basic Salary :</strong>
                            </label>
                            <p class="form-control-static">{{ $total_salary ? env('APP_CURRENCY').' '.$total_salary : '' }}</p>
                        </div>
                        <!-- <div class="">
                ...
                <div class="col-sm-6 form-horizontal">
                    <div class="panel panel-custom">
                        ...
                        <div class="panel-body">
                            <?php $allowances = $salary->allowance; ?>
                            @foreach($allowances as $ak => $av)
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>{{ $av->salary_payment_allowance_label }} : </strong></label>
                                <p class="form-control-static">{{ $av->salary_payment_allowance_value ? env('APP_CURRENCY').' '.$av->salary_payment_allowance_value : '' }}</p>
                            </div>
                            <?php $total_allowance += $av->salary_payment_allowance_value; ?>
                            @endforeach
                ...
                <div class="col-sm-6 form-horizontal">
                    <div class="panel panel-custom">
                        ...
                        <div class="panel-body">
                            <?php $deductions = $salary->deduction; ?>
                            @foreach($deductions as $dk => $dv)
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>{{ $dv->salary_payment_deduction_label }} : </strong></label>
                                <p class="form-control-static">{{ $dv->salary_payment_deduction_value ? env('APP_CURRENCY').' '.$dv->salary_payment_deduction_value : '' }}</p>
                            </div>
                            <?php $total_deduction += $dv->salary_payment_deduction_value; ?>
                            @endforeach
                    ...
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <div class="">
                                ...
                                <label class="col-sm-6 control-label"><strong>Gross Allowance: </strong></label>
                                <p class="form-control-static">{{ env('APP_CURRENCY').' '.($salary->total_allowance) }}</p>
                            </div>
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>Gratuity Salary: </strong></label>
                                <p class="form-control-static">{{ env('APP_CURRENCY').' '.($salary->gratuity_salary) }}</p>
                            </div>
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>Gross Salary: </strong></label>
                                <p class="form-control-static">{{ env('APP_CURRENCY').' '.($total_salary + $salary->total_allowance +
                                    $salary->gratuity_salary) }}</p>
                            </div>
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>Total Deduction: </strong></label>
                                <p class="form-control-static">{{ $salary->total_deduction ? env('APP_CURRENCY').' '.$salary->total_deduction : '' }}</p>
                            </div>
                            @if($salary->fine_deduction != '' && $salary->fine_deduction != '0')
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>Fine Deduction: </strong></label>
                                <p class="form-control-static">{{ $salary->fine_deduction ? env('APP_CURRENCY').' '.$salary->fine_deduction : '' }}</p>
                            </div>
                            @endif
                            <div class="">
                                <label class="col-sm-6 control-label"><strong>Net Salary : </strong></label>
                                <p class="form-control-static">{{ env('APP_CURRENCY').' '.($total_salary  - $salary->total_deduction) }}</p>
                            </div>
                             <div class="">
                                <label class="col-sm-6 control-label"><strong>Overtime : </strong></label>
                                <p class="form-control-static">{{ env('APP_CURRENCY').' '.($salary->overtime) }}</p>
                            </div>
                             <div class="">
                                <label class="col-sm-6 control-label"><strong>Total : </strong></label>
                                <p class="form-control-static">{{ env('APP_CURRENCY').' '.($total_salary + $salary->total_allowance - 
                                    $salary->total_deduction - $salary->fine_deduction + $salary->overtime +
                                    $salary->gratuity_salary) }}</p>
                            </div>
                        </div>
                    </div>
