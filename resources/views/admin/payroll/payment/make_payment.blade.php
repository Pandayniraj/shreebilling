...
<div id="EmpprintReport">
    <div class="row">
                     ...
                     ?>
                            ...
                            <div class="form-group mt">
                                <div class="col-sm-5">
                                    ...
                                    <button type="submit" name="sbtn" value="1" class="btn btn-primary btn-block">Pay </button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        @endif
        <div class="@if(!sizeof($payment)) col-sm-10 @else col-sm-12 @endif std_print">
            <div class="panel panel-custom" style="padding: 10px;">
                ...
                <table class="table table-striped DataTables  dataTable no-footer dtr-inline" id="DataTables">
                  <thead>
                     <tr>
                        <td class="text-bold col-sm-1">Month</td>
                        <td class="text-bold">Date</td>
                        <td class="text-bold">Gross Salary</td>
                        <td class="text-bold">Total Allowance</td>
                        ...
                        <td class="text-bold col-sm-2">Details</td>
                     </tr>
                  </thead>
                  <tbody>
                    @foreach($payments as $pk => $pv)
                     <tr>
                        <td class="col-sm-1">{{ date('F-Y', strtotime($pv->payment_month)) }}</td>
                        <td>{{ $pv->paid_date }}</td>
                        <td>{{ $pv->gross_salary ? env(APP_CURRENCY).' '.$pv->gross_salary : '' }}</td>
                        <td>{{ $pv->total_allowance ? env(APP_CURRENCY).' '.$pv->total_allowance : '' }}</td>
                        <td>{{ env(APP_CURRENCY).' '.$pv->total_deduction }}</td>
                        <?php $net_salary = $pv->gross_salary + $pv->total_allowance - $pv->total_deduction; ?>
                        <td>{{ env(APP_CURRENCY).' '.($pv->gross_salary - $pv->total_deduction)  }}</td>
                        <td>{{ $pv->fine_deduction ? env(APP_CURRENCY).' '.$pv->fine_deduction : '' }}</td>
                        <td>{{ $pv->overtime ? env(APP_CURRENCY).' '.$pv->overtime : '' }}</td>
                        <td>{{ env(APP_CURRENCY).' '.($net_salary - $pv->fine_deduction + $pv->overtime) }}</td>
                        <td style="text-align:left;">
                           <a href="/admin/payroll/salary_payment_detail/{{ $pv->salary_payment_id }}" class="btn btn-info btn-xs" title="" data-toggle="modal" data-target="#modal_dialog"><i class="fa fa-list"></i></a>
                        </td>
                     </tr>
                     @endforeach
                  </tbody>
                 ...
                 <span class="col-md-6 pull-right">
                         Change Staff for the month
                      {!! Form::select('user_id', $users, \Request::segment(4), ['class' => 'form-control input-sm','id' => 'user_id']) !!}
                  </span>
