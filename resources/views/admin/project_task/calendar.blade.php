...
<section class="content-header" style="margin-top: -35px; margin-bottom: 20px">
  <h1>
                Project Calendar
                <small>Tasks created here are available in the corresponding projects
                </small>
            </h1>

            <span class="pull-right">
<select class="form-control searchable" id='projects-id'>
     <option value="">Select Project</option>
      @foreach ($projects as $p)
          <option value="{{ $p->id }}" @if($selected_project && $selected_project == $p->id) selected @endif>{{ ucfirst($p->name) }}</option>
      @endforeach
...
<p></p>
    ...
    <!-- Main content -->
      ...
      <div class="row">
        <div class="col-md-3" style="@if(!$selected_project) display: none @endif">
          <div class="box box-solid">
            <div class="box-header with-border">
              <h4 class="box-title">Task Waitlist</h4>
            </div>
            <div class="box-body">
              <!-- the events -->
              <div id="external-events">
                @foreach($pendingTask as $pt)
                      <div class="external-event" data-id='{{ $pt->id }}' style="background-color: {{ $pt->color ?? 'rgb(0, 86, 179)'}} ;color:white" data-end = '{{ $pt->end_date }}' title="{{ date('dS M D h:i') }}">
                        {{ $pt->subject }}
                      </div>
                @endforeach
              </div>
        ...
        <!-- /.col -->
        <div class="@if(!$selected_project) col-md-12 @else col-md-9 @endif">
...
function handleChange(task_id, start_date) {
    $.post("/admin/ajaxTaskUpdate", {
            id: task_id,
            update_value: start_date,
            type: 'start_date',
...
function handleDragChange(task_id, start_date,end_date) {
    ...
    $('#calendar').fullCalendar({
      ...
      if(info.start_date)
           $(element).tooltip({
            html:true,
            title: `Start Date: ${info.start_date}<br>Description: ${info.description || 'Empty'}
            ${!project_id?'<br>Project:'+info.group_name:''}`,
            placement: 'top',
            trigger: 'hover',
            container: 'body'
          });
      ...
      if (info.url) {
          window.open(info.url, '_blank', 'toolbar=yes, scrollbars=yes, resizable=yes, top=0,left=100,width=999, height=660');
        }
      } 
...
function addTask(paramObj){
    ...
    paramObj['ispostedfromcalandar'] = 1;
      $.post("/admin/project_tasks/store/modals", paramObj, function(result) {
        if (result) {
            var event = $('<div />')
              ...
              event.attr('data-id',result.id)
              event.attr('data-end',paramObj.end_date)
              event.html(paramObj.subject)
    ...
    $('#add-new-event').click(function (e) {
      ...
      var task_end_date =  $('#new-event-date').val();
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Create Project Task
          <small class='start_dates'></small>
        </h4>
      </div>
      <form method="post" action="{{ route('admin.project_task.modals',$selected_project) }}">
        {{ csrf_field() }}
      <div class="modal-body">
        <div class="form-group">
          <label>Title</label>
            <input type="text" class="form-control title" placeholder="Enter ..." name='subject'>
        </div>
        @if($selected_project)
        ...
        <div class="form-group">
         <label>Select Project</label>
          <select class="form-control searchable-modal" name="project_id">
            <option value="">Select Project</option>
            @foreach ($projects as $p)
            <option value="{{ $p->id }}" @if($selected_project && $selected_project == $p->id) selected @endif>{{ ucfirst($p->name) }}</option>
            @endforeach
        ...
        @endif
        <input type="hidden"  name='start_date' class="task_start_date">
        <div class="form-group">
          <label>End Date</label>
            <input type="text" class="form-control task_end_date" placeholder="Enter ..." name="end_date" >
        </div>
         <div class="form-group">
