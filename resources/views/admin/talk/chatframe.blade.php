...
<head><meta charset='UTF-8' name="csrf-token" content="{{ csrf_token() }}" ><meta name="robots" content="noindex"><link rel="shortcut icon" type="image/x-icon"  /><link rel="mask-icon" type="" href="//production-assets.codepen.io/assets/favicon/logo-pin-f2d2b6d2c61838f7e76325261b7195c27224080bc099486ddd6dccb469b8e8e6.svg" color="#111" /><link rel="canonical" href="https://codepen.io/emilcarlsson/pen/ZOQZaV?limit=all&page=74&q=contact+" />
...
<div id="frame">
	<div id="sidepanel">
        ...
        @if(!is_null($inbox->thread))
				<li class="contact">
					...
					<div class="wrap" 
            onclick="window.open(`{{route('message.read', ['id'=>$inbox->withUser->id])}}`,'_self')">
						<span class="contact-status  @if(Cache::has('user-is-online-' .$inbox->withUser->id)) online  @else offline @endif"></span>
						<img class="contacts-list-img"  src="{{$inbox->withUser->avatar}}" style="height: 40px; width: 40px;" alt="avatar">
						<div class="meta">
							<p class="name">{{$inbox->withUser->first_name}}&nbsp;{{$inbox->withUser->last_name}}</p>
							<p class="preview">{{substr($inbox->thread->message, 0, 20)}}
	...
	<div class="content">
		<div class="contact-profile">
			<img src="{{$rcv_profile_img}}" alt="" />
			<p>{{$rcv_user->first_name}} {{$rcv_user->last_name}}</p>
    ...
    <div id='userlivechatbox'>
		<div class="messages msg_history"  id='chathistory'  data-total-message = "{{ $totalMessage }}" data-receiver="{{$receiver}}">
			<ul>
        @if($rcv_user)
      <span  v-for = '(message,index) in messageobj'  :key='message.id'  v-if='isLoaded' class="chatbox">
         
				<li class="sent"  v-if='message.sender.id == authuser' v-bind:id='"message-"+message.id'>
					<img :src="sender_profile_img" alt="" />
					<p v-if = 'message.attachment'> 
            <img v-if ="message.att_type == 'img' " :src='message.attachment' style="width: 100%; height: 100%; border-radius: 0px;" data-enlargable>
        ...
        <li class="replies" v-if='message.sender.id != authuser' v-bind:id='"message-"+message.id' >
          <img :src="sender_profile_img" alt="" />
          <p v-if = 'message.attachment'> 
            <img v-if ="message.att_type == 'img' " :src='message.attachment' style="width: 100%; height: 100%; border-radius: 0px;" data-enlargable>
              <a v-else :href="message.attachment" target="_blank" :download="message.file_name">@{{message.file_name}}</a>
