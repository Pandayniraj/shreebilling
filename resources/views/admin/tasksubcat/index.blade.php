            ...
            {!! Form::open( array('route' => 'admin.tasks.enable-selected', 'id' => 'frmClientList') ) !!}
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        &nbsp;
                        <a class="btn btn-default" href="{!! route('admin.tasksubcat.create') !!}" title="{{ trans('admin/knowledge/general.button.create') }}">
                            <i class="fa fa-edit"></i> Create Tasks Sub Category
                        </a>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered" id="knowledgecat-table">
                                ...
                                <tbody>
                                    ...
                                    @foreach($tasksubcat as $k)
                                        <tr>

                                            <td class="" align="center">{!! Form::checkbox('chkClient[]', $k->id); !!}</td>
                                            <td class="lead"> {!! $k->name !!}</td>

                                            <td >{{ $k->tasksubcat->name }}</td> 
                                          
 
                                            <td >{{ date('dS M Y', strtotime($k->created_at)) }}</td> 
                                            
                                            <td >
                                                @if ( $k->isEditable() || $k->canChangePermissions() )
                                                    <a href="{!! route('admin.tasksubcat.edit', $k->id) !!}" title="{{ trans('general.button.edit') }}"><i class="fa fa-edit"></i></a>
                                                @else
                                                    <i class="fa fa-edit text-muted" title="{{ trans('admin/k/general.error.cant-edit-this-document') }}"></i>
                                                @endif

                                                @if ( $k->isDeletable() )
                                                    <a href="{!! route('admin.tasksubcat.confirm-delete', $k->id) !!}" data-toggle="modal" data-target="#modal_dialog" title="{{ trans('general.button.delete') }}"><i class="fa fa-trash deletable"></i></a>
                                                @else
