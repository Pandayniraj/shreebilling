<table style="border: 1px solid; text-align:center">
    <tr>
        <th colspan="16" style="text-align:center; font-size:16px font-weight:bold;">Jai Shreebinayak Distributor and
            Trade Link Pvt.Ltd</th>
    </tr>
    <tr>
        <th colspan="16" style="text-align:center; font-size:14px font-weight:800;">Chamati-15, Kathmandu, Nepal</th>
    </tr>
    <tr>
        <th colspan="16" style="text-align:center; font-size:14px font-weight:800;">Daily Dispatch Sheet and Stock of
            {{ \App\Helpers\TaskHelper::getNepaliDate(date('Y-m-d')) }}</th>
    </tr>
    ...
    <table class="table table-bordered">
        ...
        <tr>
            ...
            @foreach ($products as $product)
                ...
                $openingvalue = 0;
                ?>
                @if(isset($foropening[$product->id][0]->removestock) &&
        ...
        <tr>
            ...
            @foreach ($products as $product)
                @if ($perdaypurchasedetail[$product->id][0]->totalpurchaseqty)
                    <?php
                    $totalpurchase += (float) $perdaypurchasedetail[$product->id][0]->totalpurchaseqty;
                    ?>
                    <td>
        ...
        <tr>
            ...
            @foreach ($products as $product)
                @if ($perdayproducttotal[$product->id][0]->totalproductqty)
                    <?php
                    $totaldispatch += (float) abs($perdayproducttotal[$product->id][0]->totalproductqty);
                    ?>
                    <td>
        ...
        <tr>
            ...
            @foreach ($products as $product)
                ...
                $finalclosingvalue = 0;
                ?>
                @if (isset($asopeningstock[$product->id][0]->asopeningstock) &&
                        ($asopeningstock[$product->id][0]->asopeningstock > 0 || $asopeningstock[$product->id][0]->asopeningstock == 0))
                    @if (
                ...
                @if ($openingvalue && $openingvalue != '')
                    @if (
                        $perdaypurchasedetail[$product->id][0]->totalpurchaseqty == null ||
                            $perdaypurchasedetail[$product->id][0]->totalpurchaseqty == '')
                        <?php
                    ...
                    @else
                        ...
                        ?>
                    @endif
                    <?php $totalasclosing += (float) $finalclosingvalue;
                    $totalclosingvalue += (float) $finalclosingvalue; ?>
                ...
                @elseif($perdaypurchasedetail[$product->id][0]->totalpurchaseqty)
                    ...
                    ?>
                    <td>{{ $perdaypurchasedetail[$product->id][0]->totalpurchaseqty - abs($perdayproducttotal[$product->id][0]->totalproductqty ?? 0) }}
                    </td>
                @else
                    <td>0</td>
                @endif
